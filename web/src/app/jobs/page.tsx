...
export default function JobsPage() {
  ...
  const load = useCallback(async () => {
    ...
    setError(null);
    try {
      const r = await api.jobs({ kind: kind || undefined, limit, offset });
      setData(r.data);
    } catch (e) {
      setError(e);
    } finally {
          ...
          action={
            <div className="flex flex-wrap items-center gap-3">
              <label className="flex items-center gap-2 text-sm text-slate-600">
                {t("jobs.kind")}
                <Select value={kind} onChange={(e) => { setKind(e.target.value); setOffset(0); }} className="w-auto">
              ...
              <label className="flex items-center gap-2 text-sm text-slate-600">
                {t("jobs.pageSize")}
                <Select value={limit} onChange={(e) => { setLimit(Number(e.target.value)); setOffset(0); }} className="w-auto">
                  {PAGE_SIZES.map((n) => (
                ...
                <tbody className="divide-y divide-slate-100 bg-white">
                  {data.items.map((j) => (
                    <tr
                      ...
                      onClick={() => router.push(`/jobs/${encodeURIComponent(j.job_id)}`)}
                      tabIndex={0}
                      onKeyDown={(e) => {
                        if (e.key === "Enter") router.push(`/jobs/${encodeURIComponent(j.job_id)}`);
                      }}
                    >
                      <td className={td}>
                      ...
                      <td className={td}>{KINDS.includes(j.kind as (typeof KINDS)[number]) ? t(`jobs.kind.${j.kind}` as MessageKey) : j.kind}</td>
